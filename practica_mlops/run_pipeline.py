...
# ── Exportar CSV de métricas globales ──
csv_global = os.path.join(REPORTS_DIR, "metricas_globales.csv")
...
df_global = pd.DataFrame({
    ...
    "fix": [
        metricas_fix["accuracy"] if fix_applied else None,
        metricas_fix["precision"] if fix_applied else None,
        metricas_fix["recall"] if fix_applied else None,
        metricas_fix["f1"] if fix_applied else None
    ],
    "equipo": [team_name]*4
})
 
df_global.to_csv(csv_global, index=False)
print(f"CSV global guardado en: {csv_global}")
 
 
# ── Exportar CSV de curva (Power BI) ──
csv_curve = os.path.join(REPORTS_DIR, "metricas_powerbi.csv")
 
df_curve = pd.DataFrame({
    "semana": semanas,
    "accuracy_base": acc_curve,
    "f1_base": f1_curve,
    "accuracy_fix": acc_fix_curve if fix_applied else [None]*len(semanas),
    "f1_fix": f1_fix_curve if fix_applied else [None]*len(semanas),
    "equipo": [team_name]*len(semanas)
})
 
df_curve.to_csv(csv_curve, index=False)
print(f"CSV curva guardado en: {csv_curve}")
